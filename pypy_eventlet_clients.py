...
class DevSocket():
    def __init__(self,pa):
        ...
        print "connect",self.fileno
        self.sbuf = self.device_struct_allocate()
        self.socket_write()
        #self.run()
        #pool.spawn_n(self.run)



    def run(self):
        print "start socket run"
        while True:
            n = time.time()
            print "run socket loop"
            eventlet.hubs.get_hub().switch()
            try:
                data = self.sock.recv(SOCK_BUFSIZE)
            except socket.timeout:
                self.errqueue.put('sock %d recv timeout %f' % (self.fileno,time.time()-n))
                break
            except IOError:
                self.errqueue.put('sock %d recv IOError ' % (self.fileno))
                break
            if not data:
                self.errqueue.put('sock %d recv recv no data,maybe disconnect' % (self.fileno))
                break
...
def DevDemo(args):
    ...
    eventlet.spawn_n(WorkerThread,errqueue,errlog)
    eventlet.spawn_n(WorkerThread,statqueue,statlog)
    ...
    eventlet.spawn_n(DevGreenPool)
    for uid in uulist:
        sockqueue.put_nowait((uid,host))
        #print uid
        eventlet.sleep(1)
