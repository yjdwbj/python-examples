...
def handle_client_request(buf,fileno):
    ...
    if res.method == STUN_METHOD_SEND or res.method == STUN_METHOD_DATA:
        ...
        if gClass.clients.has_key(dstsock):
            # 转发到时目地
            handle_send_responses(buf,dstsock)
            return (None,res)
        else: # 目标不存在
...
def bind_each_uuid(ustr,fileno):
    ...
    if gClass.devuuid.has_key(ustr):
        ...
        handle_send_responses(notify_peer(''.join([b,STUN_ONLINE])),gClass.clients[dstsock])
    ...
    if not row:
        ...
        if rlist[3] and gClass.devuuid.has_key(huid): # 下面是发一个包给小机，确认它一定在线,收到对方确认之后再回复APP
            ...
            try:#这里先去告诉小机，有一个客户端要连接它
                ...
                epoll.modify(sock,select.EPOLLOUT | select.EPOLLET)
            except IOError:
    ...
    if not row:
        ...
        if rlist[3] and gClass.devuuid.has_key(huid): # 下面是发一个包给小机，确认它一定在线,收到对方确认之后再回复APP
            ...
            try:#这里先去告诉小机，有一个客户端要连接它
                ...
                asktimer.start()
                #gClass.responses[sock] = stun_connect_address(res.host,res)
                handle_send_responses(stun_connect_address(res.host,res),gClass.clients[sock])
...
def noify_app_uuid_just_login(fileno,uuidstr,devsock):
    handle_send_responses(notify_app_bind_islogin(''.join([uuidstr,'%08x' % devsock])),gClass.clients[fileno])
    #gClass.responses[fileno] = notify_app_bind_islogin(''.join([uuidstr,'%08x' % devsock]))
    #epoll.modify(sock,select.EPOLLOUT | select.EPOLLET)
...
def notify_peer_is_logout(fileno,dstsock):
    ...
    handle_send_responses(notify_peer(''.join(['%08x' % dstsock,STUN_OFFLINE])),gClass.clients[fileno])
    return
    try:
...
def sock_send_fail(fileno):
    ...
    if phead.method == STUN_METHOD_SEND or phead.method == STUN_METHOD_DATA:
        ...
        handle_send_responses(''.join(tbuf),gClass.clients[srcsock])
        ...
        while True:
            ...
            for fileno,event in events:
                if fileno == srvsocket.fileno():
                    ...
                    epoll.register(nf,select.EPOLLIN | select.EPOLLET)
...
def CServer(conn):
    ...
    self.srvsocket.bind(conn)
    self.srvsocket.listen(5)
    stackless.tasklet(acceptConn)(srvsocket)

def acceptConn(srvsocket):
    while srvsocket.accept:
        (csocket,addr) = srvsocket.accept()
        stackless.tasklet(manageSocket)(csocket,addr)
        stackless.schedule()

def manageSocket(csocket,addr):
    gClass.clients[csocket.fileno()] = csocket
    while csocket.connect:
        recvbuf = csocket.recv(BUFSIZE)
        if not recvbuf:
            break
        hbuf = binascii.hexlify(recvbuf)
        if cmp(hbuf[:4],HEAD_MAGIC): # 检查JL关键字
        ...
        if (len(hbuf)/2)  > int(hbuf[8:12],16):
            ...
            mplist.reverse()
            #print "two packet",mplist
            #[stackless.tasklet(handle_requests_buf(n,fileno),csocket) for n in mplist]    
            [handle_send_responses(handle_requests_buf(n,fileno),csocket) for n in mplist]    
        else:
            handle_send_responses(handle_requests_buf(hbuf,fileno),csocket)
        stackless.schedule()
    csocket.close()

def handle_send_responses(hbuf,socket):
    if not hbuf:
        return
    nbyte =  socket.send(binascii.unhexlify(''.join(hbuf)))
    log.info(','.join(['sock %d' % fileno,'send: %d'%nbyte]))
